...
int read_user(FILE *fptr, char username[MAX_STR_LEN], char password[MAX_STR_LEN], char user_role[MAX_STR_LEN], char next_line[MAX_STR_LEN]){
	...
	/*preparo buffer temporanei ----------------*/
	char buff_username[MAX_STR_LEN+2];
	char buff_password[MAX_STR_LEN+2];
	char buff_user_role[MAX_STR_LEN+2];



    /*leggo i dati------------------------------*/
    if  (fgets(buff_username,MAX_STR_LEN+2,fptr) == NULL  ||
        fgets(buff_password,MAX_STR_LEN+2,fptr) == NULL  ||
        fgets(buff_user_role,MAX_STR_LEN+2,fptr) == NULL ) 
    ...
    buff_user_role[strcspn(buff_user_role,"\n")] = '\0';

	/*copio le stringhe finali, sicure nelle stringhe di ritorno*/
	strcpy(username,buff_username);
	strcpy(password,buff_password);
	strcpy(user_role,buff_user_role);
